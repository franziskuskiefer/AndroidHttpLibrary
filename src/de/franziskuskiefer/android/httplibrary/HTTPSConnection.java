...
public class HTTPSConnection extends AsyncTask<String, Void, HashMap<String, String>> {
	...
	protected HTTPSConnection(Callback callback, boolean app){
		this.caller = callback;
	}
	
	protected HTTPSConnection(Callback callback, boolean app, String textParams){
		this(callback, app);
	 ...
	 */
	protected HTTPSConnection(Callback callback, boolean app, HashMap<String, String> params) {
		this(callback, app);
	...
	protected HashMap<String, String> doInBackground(String... urls) {
		...
		try {
			// the second string is actually an authentication cookie
			if (urls.length > 1)
				return getRequest(urls[0], urls[1]);
			else
				return getRequest(urls[0], null);
		} catch (IOException e) {
	...
	private HashMap<String, String> getRequest(String myurl, String auth) throws IOException {
		...
		if (myurl.toLowerCase(Locale.UK).startsWith(HTTPS)) {
				...
				String user = "user";
				String pwd = "pwd";
				HostnameVerifier verifier = MyHostnameVerifier.getInstance(user, pwd);
